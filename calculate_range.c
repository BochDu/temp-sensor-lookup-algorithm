...
#define SAR_MAX 1023
#define DIVIDER_RESISTRANCE 4700
...
int main()
{
    ...
    for (adc_value = 0; adc_value < SAR_MAX; adc_value++)
    {
        float rt, temp;
        rt = adc_value * DIVIDER_RESISTRANCE / (SAR_MAX - adc_value);
        rt = rt / 1000;
        temp = binarySearch(data, data_size, rt);
        printf("adc_value: %d,rt: %f,temp: %f\n", adc_value, rt, temp);
    }
    return 0;
}
